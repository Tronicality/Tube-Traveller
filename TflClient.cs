...
{
    internal class TflClient //Ignore the whole Client
    {
        ...
        {
            Stream stream = await _client.GetStreamAsync(@"https://api.tfl.gov.uk/stationdata/tfl-stationdata-detailed.zip"); 
            return stream;
        }
        ...
        public async Task<List<Root>> GetTest(string id)
        {
            try
            {
                var responseBody = await _client.GetStringAsync($"https://api.tfl.gov.uk/Line/{id}/StopPoints");
                var root = JsonConvert.DeserializeObject<List<Root>>(responseBody);
                return root!;
            }
            catch (Exception ex)
